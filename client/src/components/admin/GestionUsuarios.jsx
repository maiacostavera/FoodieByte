 ...
 */
function GestionUsuarios({ usuarios, alCambiarRol, alRechazarSolicitud, alCambiarActivacion }) {
    const [solicitudAbierta, setSolicitudAbierta] = useState(null);

    const esPostulante = (u) => u.activo && u.solicitud_vendedor && u.rol === 'foodie';
        ...
        <div style={estilos.panelBlanco}>
            ...
            <div style={{ overflowX: 'auto' }}>
                <table style={estilos.tabla}>
                    ...
                    <tbody>
                        {usuarios.map(u => (
                            <tr key={u.id} style={{ ...estilos.filaBody, ...(u.activo ? {} : estiloFilaInactiva) }}>
                                <td style={{ ...estilos.celdaBody, fontWeight: '600' }}>#{u.id}</td>
                                <td style={{ ...estilos.celdaBody, fontWeight: '500', color: '#212121' }}>
                                    {u.nombre}
                                    {esPostulante(u) && <span style={estiloBadgePostulante}>POSTULANTE</span>}
                                    {!u.activo && <span style={estiloBadgeInactiva}>DESACTIVADA</span>}
                                </td>
                                ...
                                <td style={estilos.celdaBody}>
                                    {esPostulante(u) && (
                                        <>
                                            {/* Los datos del formulario de alta se guardan y se
                                    ...
                                    {u.activo ? (
                                        <button onClick={() => alCambiarActivacion(u.id, false)}
                                            style={{ ...estilos.botonEliminar, ...estiloBotonBloque, marginBottom: 0 }}>
                                            Desactivar
                                        </button>
                                    ) : (
                                        <button onClick={() => alCambiarActivacion(u.id, true)}
                                            style={{ ...estilos.botonEditar, backgroundColor: '#2e7d32', ...estiloBotonBloque, marginBottom: 0 }}>
                                            Reactivar
                                        </button>
                                    )}
                                </td>
...
const estiloBadgeInactiva = { marginLeft: '8px', backgroundColor: '#eeeeee', color: '#616161', padding: '4px 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: '600' };
const estiloFilaInactiva = { backgroundColor: '#fafafa', opacity: 0.75 };
